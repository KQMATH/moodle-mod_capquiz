...
class capquiz_question_engine
{
    ...
    {
        if ($attempt = capquiz_question_attempt::active_attempt($this->capquiz, $user)) return false;
        else if ($question = $this->find_question_for_user($user)) return false;
        return true;
    }

    public function attempt_for_user(capquiz_user $user)
    {
        if ($attempt = capquiz_question_attempt::active_attempt($this->capquiz, $user)) return $attempt;

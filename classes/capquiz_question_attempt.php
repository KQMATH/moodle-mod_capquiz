...
class capquiz_question_attempt
{
    ...
    {
        global $DB;
        $criteria = [database_meta::$field_user_id => $user->id(), database_meta::$field_reviewed => false];
    ...
    {
        global $DB;
        $criteria = [database_meta::$field_id => $attempt_id, database_meta::$field_user_id => $user->id()];
        if ($entry = $DB->get_record(database_meta::$table_capquiz_attempt, $criteria)) {
            return new capquiz_question_attempt($capquiz->question_usage(), $entry);
        }
        return null;
    ...
    public static function inactive_attempts(capquiz $capquiz, capquiz_user $user)
    {
        global $DB;
        $records = [];
        $criteria = [database_meta::$field_user_id => $user->id(), database_meta::$field_answered => true, database_meta::$field_reviewed => true];
